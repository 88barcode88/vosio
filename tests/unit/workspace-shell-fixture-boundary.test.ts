...
describe("workspace shell fixture boundary", () => {
  it("recognizes only the development-only fixture namespace", () => {
    ...
    expect(isDevelopmentWorkspaceShellFixture("/login/soniox-installation-e2e", "development")).toBe(true);
    ...
    expect(isDevelopmentWorkspaceShellFixture("/login/soniox-installation-e2e", "production")).toBe(false);
  });
});
