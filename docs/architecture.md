...
Čtyři forward migrace jsou v této fázi pouze v source a musí se aplikovat v přesném pořadí: `20260804100000_add_evidence_locations.sql`, `20260804110000_add_recording_organization.sql`, `20260804120000_add_recording_markers.sql`, `20260804130000_add_transcript_fulltext_search.sql`; potom musí proběhnout úspěšný DB postflight na každé cílové databázi a teprve následně deploy aplikace. Žádná z nich nebyla parsovaná ani aplikovaná v disposable, staging nebo live DB. Neproběhlo skutečné ověření SQL/PG15, GIN indexu ani `EXPLAIN`, evidence/organization/marker/search constraintů, grantů, forced RLS, anon-vs-auth a dvouuživatelské izolace, latest-transcript current-vs-old chování, raw/manual fallbacku, deleted filtru, runtime keyset/offset stránkování ani backfillu. Deploy kódu používajícího kterýkoli z těchto čtyř kontraktů je blokovaný do explicitně schváleného apply a úspěšného postflightu všech cílových DB.
