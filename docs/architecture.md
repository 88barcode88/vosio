...
Vosio je cílově robustní Next.js PWA pro audio nahrávky, přepisy a AI zpracování. Aplikace je self-hostovatelná v běžném Next.js runtime, používá Supabase pro auth, databázi a storage, Soniox pro speech-to-text a OpenAI, Google Gemini a Mistral přes server-side AI adaptéry.
...
| OpenAI API / Gemini API / Mistral API | AI zpracování transcriptů přes server-side provider adapter |
...
Jediná platná gramatika části je `part-000000.webm` nebo `part-000000.m4a`, vždy lowercase, šest číslic a index od nuly. Shared validator ignoruje jiné objekty, numericky seřadí platné části a při duplicitě, mixu přípon nebo mezeře fail-closed vrací z recovery či transcription cesty bezpečný konflikt `409`. Provider create failure nebo explicitní terminal failure status selže pouze odpovídající transcription job nebo celý aktuální segmentový batch; recording se vrátí do `uploaded` bez změny audio metadat, takže další explicitní retry založí nový pokus. Přechodná chyba provider poll requestu nebo ukládání jeho průběžného statusu nechá existující job či batch v dosavadním retryable stavu a další poll může pokračovat.
...
Inbox nahrávek volí řádky nebo skládané karty podle skutečné inline šířky panelu, ne jen podle šířky viewportu. Nad 680 px obsahu zůstávají desktopové sloupce s pružným názvem, metadaty a úzkou ellipsis akcí; editor názvu a Koš se zobrazují až v jejím přístupném menu. Při 680 px a méně se řádek skládá jako karta. Chyba soft-delete se v obou režimech vykreslí jako rezervovaný blok přes celou šířku pod hlavním obsahem, takže nepřekrývá další řádek.
...
Detail nahrávky nenačítá AI bodies ani projekce v serverovém critical path. Transcript-scoped klient načítá owner-scoped metadata až pro AI, Timeline nebo export, nejvýše 50 manuálních job summaries a samostatně nejvýše šest automatických jobů aktuální generace, plus 50 output metadata na stránku bez promptu, provider configu či transcriptu. Těla se načítají podle potřeby; export projde celou historii v omezených dávkách. Stavový poll používá serverové `poll_eligible`, 10 s prvních 30 s, 30 s mezi 30 a 120 s a 60 s poté, pouze visible/online relevantní plochu, nejvýše jeden request a backoff po chybě nejméně 30 s. Lifecycle catch-up nastane jen více než 30 s po dokončené kontrole. Same-transcript server props zachovávají cache a DOM; explicitní potvrzená ID smazání/mutace se slučují lokálně s tombstones, staré odpovědi odděluje generation fence. Žádný kompletní `router.refresh` při těchto operacích.
...
Settings má šest nezávislých automatických výstupů. Nová preference `automaticOutputTypes` vybírá shrnutí, úkoly, zápis, CRM poznámku a navazující e-mail; výchozí prázdný seznam nikdy nepřebírá souhlas ze starých dormant polí. Samostatný existující `autoTimelineAfterTranscription` dál řídí časovou osu. Vše vypnuto znamená žádné nové automatické AI. Regular/segmented STT, live, import i obnova uloženého live textu volají jediný `complete_transcript_generation_v2`: pod zámkem transcriptu společně uloží completion identity, stav `completed` a pro každý vybraný typ přesný immutable prompt/model/provider snapshot. Jakákoli chyba vrací celý přechod zpět. Partial drafts, změna nastavení a otevření historického přepisu nevytvářejí nové intenty. Stejná generace zachová původní rozhodnutí; v1 timeline wrapper zachová starou signaturu a digest.

Po commitu se nejvýše šest jobů nezávisle zařadí z uložených intentů. Provider fan-out běží přes Next `after()`, s 240s provider timeoutem, 300s route budgetem a 900s lease; host termination není garance dokončení. Obnova při aktivaci AI/Timeline nebo způsobilém pollu čte pouze současné intenty. Automatická publikace používá `publish_automatic_ai_output_v2`: zamkne transcript a job, před jakýmkoli zápisem ověří generaci, vlastníka, typ a platný lease, pak atomicky uloží raw output, serverem odvozené projekce, usage a done settlement. SQL chyba projekce nesmí být spolknutá. Úspěšný či terminální job se nespouští znovu ani po smazání outputu. Manuální protokol zůstává samostatný. [Přesný rollout a ověření](api/automatic-ai-outputs.md).
...
- AI provider adapter: OpenAI, Gemini a Mistral jako server-side provider adaptéry
...
Desktopový sidebar je 224 px rozbalený a 64 px sbalený; netajná preference railu se ukládá lokálně. Pracovní obsah začíná 72px globálním topbarem s breadcrumbem a přepínačem motivu. Na šířce 900 px a méně zůstává přesně pět cílů spodní mobilní navigace.
...
Na desktopu nad 900 px používá workspace 224px sticky levý sidebar a pracovní plochu přes celou zbývající šířku viewportu s responzivním vnitřním odsazením, bez globálního max-width stropu. Primární část obsahuje v pořadí `Nahrávky`, `Nová nahrávka` a `AI prompty`; spodní utility část obsahuje `Koš`, `Nastavení`, `Dokumentace`, nezměněný externí odkaz `Kup mi kafe` a účet uživatele. Přepínač motivu je pouze v pravé části globálního topbaru, ne v sidebaru. Sidebar neobsahuje globální AI položku, duplicitní seznam nahrávek ani storage kartu. Na šířce 900 px a méně se sidebar schová a fixed navigace se safe-area paddingem má přesně pět cílů: `Nahrávky`, `Nová`, `AI prompty`, `Nastavení`, `Více`. Persistentní recorder dock na stejném breakpointu dostává safe-area offset nad navigaci, takže ji nepřekrývá ani mezi 761 a 900 px. `Více` otevírá focus-managed Drawer s `Košem`, `Dokumentací`, přepínačem motivu, support odkazem a účtem s odhlášením. Drawer přebírá Escape, backdrop, focus trap a návrat focusu ze sdílené UI primitive; pending stav se po změně pathname resetuje.

Detail nahrávky je na všech šířkách jednosloupcový dokument bez dominantního vedlejšího railu. Desktopový shell používá jako jediný hlavní svislý scroll owner `.content-area-document`; na šířce 900 px a méně přechází stejný obsah do běžného scrollu dokumentu/prohlížeče. Transcript, AI výstupy, časová osa ani soubory nevytvářejí další vnořený scroll. Přilepený blok drží audio přehrávač a záložky dostupné při čtení dlouhého přepisu a na desktopu je přilepený přímo pod 72px globálním topbarem. Na mobilním breakpointu je přehrávač fixed nad spodní navigací a dokument má odpovídající rezervu, takže poslední řádek zůstane dosažitelný a žádný podporovaný viewport nevytváří horizontální posun.

Přepínač světlého a tmavého režimu je na desktopu jako icon-only akce v pravé části globálního topbaru. Volba se ukládá do `localStorage` i cookie `vosio-theme` a nastavuje `data-theme` na kořenovém `<html>` elementu. Cookie se čte už v serverovém layoutu, aby světlý režim po refreshi nezačínal krátce tmavým fallbackem.
...
Stránka `/settings` začíná částí Účet s přihlášeným e-mailem a změnou hesla, teprve potom následují produktové preference. Ukládá netajné uživatelské preference do Supabase Auth `user_metadata.vosio_settings`. Tato metadata jsou pouze preference, nesmí se používat pro autorizaci. Patří sem výchozí AI model s orientační cenou, jeho vždy viditelné vysvětlení Model a kvalita, Soniox region, výchozí Soniox realtime jazyk a model, čerstvé volby `automaticOutputTypes`, zachovaný explicitní opt-in `autoTimelineAfterTranscription` a `trashRetentionHours`. Všechny automatické výstupy jsou defaultně vypnuté; po dokončení nové transcript generation se snapshotne souhlas, výchozí model a účinný prompt pro každý zvolený typ do samostatného intentu. Nevzniká historický backfill. Pokud první enqueue selže nebo se browser zavře, detail při příštím otevření obnoví chybějící job pouze z tohoto intentu; historický transcript bez intentu zůstane `not_scheduled`. Trash retence přijímá pouze `24`, `168` nebo `720` hodin, při chybějící či neplatné hodnotě používá `720` a ovlivňuje pouze budoucí přesuny do Koše; konkrétní deleted řádek drží neměnný snapshot deadline. Část Účet mění heslo pouze po ověření aktuálního hesla v request-scoped Supabase session; nové hodnoty se neukládají do metadata ani action state. Global je výchozí Soniox region; EU volba se použije pro realtime i nové async joby a každý async job si region uloží do `provider_config`, aby další polling nezávisel na pozdější změně preference. EU vyžaduje EU-enabled Soniox projekt a odpovídající regionální klíč. Samostatné nastavení upozornění na dlouhou nahrávku není aktivní runtime funkce a nesmí se prezentovat jako ochrana nebo ukončení nahrávání. Aktuální katalog obsahuje GPT-5.6 Sol/Terra/Luna, Gemini 3.6/3.8 Flash, GPT-6 Astra Low/Medium a Mistral Small 4/Large 3; přesné provider modely, reasoning, thinking a orientační ceny jsou v `src/lib/model-options.ts`. Pouze výslovně známé legacy aliasy se normalizují podle stejné mapy, neznámá preference nespustí placený fallback. Menším a levnějším modelům může uniknout více detailů, úkolů nebo důkazů i při stejném promptu a schématu; evidence zůstává smluvně povinná, ale výsledek je nutné u důležitých callů ověřit v přepisu. Gemini vyžaduje `GEMINI_API_KEY` a Mistral `MISTRAL_API_KEY`; OpenAI modely vyžadují `OPENAI_API_KEY`. API klíče, Supabase projekt, storage bucket a bezpečnostní limity zůstávají systémové hranice řízené kódem nebo serverovou konfigurací.
...
Stránka `/recordings` je Appica-inspired inbox na neutrálním `#f7f8fa` pozadí. Hlavička má název, vysvětlení a tmavou primární akci `Nová nahrávka`; pod ní je kompaktní toolbar s hledáním, keep-mounted pokročilými filtry a `Spravovat`. Stavové facety tvoří měkký segmentovaný řádek a samotný seznam je jediný bílý 16px panel s jemným okrajem. Název je hlavní Next odkaz na detail a zároveň pružný sloupec. Editace názvu a přesun do Koše jsou schované pod samostatnou přístupnou ellipsis akcí na konci řádku, aby starý pevný action track nezabíral trvale místo. Otevřené menu nesmí vnořit interaktivní prvky do odkazu na detail. Editor se zavře až po potvrzeném uložení; změna se zapisuje přes authenticated Supabase klienta a RLS. Správa klientů, projektů, plochých složek a štítků používá keep-mounted pravý Drawer; zavření odstraní obsah z focus/accessibility stromu, ale nezahodí draft, pending save ani jeho settlement. Nad seznamem se zobrazí recovery panel, pokud existují nedokončené live drafty s uloženým partial transcriptem nebo audio segmenty. Stavový řádek používá exact serverové facety přes celý aktuální `q`, organizační filtry a ALL sadu štítků; facety ignorují aktivní `status` a samostatné `Smazáno` ukazuje úplný Koš. Existující URL-backed filtry jsou `q`, `status`, `client`, `project`, `folder` a opakovatelný `tag`; source ani date filtr tento UI-only řez nepřidává. `q` používá indexed PostgreSQL hledání v nejnovějším přepisu, názvu nahrávky, klientovi, projektu, složce a štítcích. Změna organizačních filtrů zachovává `q` i nesouvisející query parametry; vyčištění hledání zachová rozepsané organizační filtry a vyčištění organizace zachová rozepsané `q`. Neplatné, cizí, duplicitní nebo klientovi neodpovídající hodnoty se kanonizují. Více štítků má ALL sémantiku. Vyhledávání vrací stránku 25 výsledků s excerptem a bezpečným odkazem na transcript; bez `q` aplikace načítá organizační seznam stabilním keysetem `(created_at, id)`. Při viewportu 900 px a méně se skládá toolbar a `Spravovat` zabere celou šířku; seznam se nezávisle přepne z flat řádků na skládané karty při skutečné šířce content containeru 680 px a méně. Nahrávku lze z ellipsis menu soft-smazat do stavu `deleted`; tím zmizí z hlavního seznamu a objeví se na `/trash`. Koš podporuje trvalé smazání přes server action, která ověří vlastníka, odstraní soubor ze Storage bucketu `recordings` a poté smaže DB řádek; související přepisy, joby, AI výstupy a search chunks se mažou kaskádou. Mazání v UI vždy vyžaduje potvrzení a po potvrzení se řádek nebo explicitně označená search karta optimisticky schová ještě před doběhnutím server action; neočekávaný reject obnoví stejný element a zobrazí sanitizovanou chybu v rezervovaném plnošířkovém druhém řádku uvnitř řádku nebo karty. Route-level loading udržuje geometrii inboxu; route error nezobrazuje interní detail chyby a volá skutečné App Router `reset()`.
...
GitHub Actions job `check` má jediný automatický trigger `pull_request` s výchozími activity types `opened`, `synchronize` a `reopened`. `actions/checkout@v7` a `actions/setup-node@v7` používají runtime Node 24, zatímco setup aplikace zůstává na Node 22; checkout bez `ref` override ověřuje PR merge ref. Workflow nemá `push`, post-merge běh ani `workflow_dispatch`, takže přímý push do `main` není tímto workflow automaticky pokrytý. Po `npm ci` běží produkční audit, typecheck, lint a build; testy se na GitHubu nespouštějí. Lokální `npm run check` nadále zahrnuje unit testy a lokální E2E ověření zůstává součástí release gate. Job má `contents: read`, PR concurrency a timeout 20 minut.
