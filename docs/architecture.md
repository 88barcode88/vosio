...
Zdroj pravdy pro bootstrap nového Supabase projektu je celý seřazený migrační řetězec, ne samotná baseline: `20260617000000_initial_schema.sql`, potom `20260804100000_add_evidence_locations.sql`, `20260804110000_add_recording_organization.sql` a `20260804120000_add_recording_markers.sql`. Baseline vytváří core public tabulky, enumy, indexy, forced RLS, private Storage bucket `recordings`, storage policies a systémové prompt templates; každá novější migrace mění výsledné schema až po svém apply. Tři forward migrace jsou aktuálně source-only a jejich release gate níže zůstává závazný. Existující produkční projekt může mít historicky jiné položky v `supabase_migrations.schema_migrations`; rozhodující je shoda skutečného schématu s explicitně schválenou a aplikovanou částí aktuálního řetězce.
...
Tři forward migrace jsou v této fázi pouze v source a musí se aplikovat v přesném pořadí: `20260804100000_add_evidence_locations.sql`, `20260804110000_add_recording_organization.sql`, `20260804120000_add_recording_markers.sql`; potom musí proběhnout úspěšný DB postflight a teprve následně deploy aplikace. Žádná z nich nebyla parsovaná ani aplikovaná v disposable, staging nebo live DB. Neproběhlo skutečné ověření PostgreSQL 15 syntaxe pro column-list `ON DELETE SET NULL`, odloženého client `NO ACTION` při cascade smazání `auth.users`, organizačních constraintů, case-insensitive indexů, grantů, forced RLS, dvouuživatelské izolace ani runtime keyset stránkování. Deploy kódu používajícího evidence sloupce, organizační tabulky/RPC nebo `recording_markers` je blokovaný do explicitně schváleného apply a postflightu.
...
Stránka `/recordings` je čistý seznam nahrávek. Řádek nahrávky se otevírá kliknutím na hlavní část řádku a tlačítko `Otevřít` se nepoužívá. Název nahrávky lze upravit přímo v seznamu přes malý ukotvený editor v akčním sloupci; editor neroztahuje řádek, kliknutí uvnitř něj neotevírá detail nahrávky a zavře se až po potvrzeném uložení. Změna se zapisuje přes authenticated Supabase klienta a RLS. Nad seznamem se zobrazí recovery panel, pokud existují nedokončené live drafty s uloženým partial transcriptem nebo audio segmenty. Seznam má URL parametr `q` pro lehké serverové hledání v metadatech nahrávky a kanonické filtry `client`, `project`, `folder` a opakovatelný `tag`. Změna organizačních filtrů zachovává `q` i nesouvisející query parametry; vyčištění hledání zachová rozepsané organizační filtry a vyčištění organizace zachová rozepsané `q`. Neplatné, cizí, duplicitní nebo klientovi neodpovídající hodnoty se kanonizují. DB RPC používá pro více štítků ALL sémantiku a aplikace načítá všechny stránky stabilním keysetem `(created_at, id)` se stejnými filtry, deduplikuje hranice a až potom aplikuje lehké `q` přes název, stav, zdroj a MIME typ. Fulltext přes `transcripts.raw_text` se nemá přidávat jako neindexovaný scan; patří do samostatné FTS/RPC vrstvy. Nahrávku lze přes samostatné tlačítko koše soft-smazat do stavu `deleted`; tím zmizí z hlavního seznamu a objeví se na `/trash`. Koš podporuje trvalé smazání přes server action, která ověří vlastníka, odstraní soubor ze Storage bucketu `recordings` a poté smaže DB řádek; související přepisy, joby a AI výstupy se mažou kaskádou. Mazání v UI vždy vyžaduje potvrzení a po potvrzení se řádek nebo karta optimisticky schová ještě před doběhnutím server action.
