...
Izolovaná D1 safety vrstva je připravená pro navazující zapojení do centrálního recorderu, ale sama ho nemění. Rotující recorder pracuje pouze s klonem dodaného streamu, každý díl úplně zastaví a teprve potom předá finální Blob. Durable audio repository zapisuje Blob a owner/recording/generation/index/offset/MIME/size atomicky do IndexedDB před bounded promotion. Promotion načítá pouze aktuálního vlastníka, používá deterministickou cestu a `upsert=false`; existující přesný objekt znovu použije pouze po shodě velikosti a MIME. Cleanup je omezený na přesný owner/recording/generation scope.
