...
Inbox nahrávek volí řádky nebo skládané karty podle skutečné inline šířky panelu, ne jen podle šířky viewportu. Nad 680 px obsahu zůstávají desktopové sloupce s kompaktním 116px pruhem akcí; při 680 px a méně se řádek skládá jako karta a akce přecházejí pod metadata. Tím se zohlední i 248px workspace sidebar. Chyba soft-delete se v obou režimech vykreslí jako rezervovaný blok přes celou šířku pod hlavním obsahem, takže nepřekrývá další řádek.
...
Akce `Nová nahrávka` vede na samostatnou pracovní kartu `/recordings/new` se dvěma rovnocennými primárními metodami v pevném pořadí: live nahrávání přes mikrofon a upload existujícího audio souboru. Import hotového textového přepisu zůstává funkční jako vedlejší, výchozí zavřená sekce pod nimi. Obrazovka je kompaktní capture workspace, ne hero stránka. Desktop nad 900 px drží primární karty vedle sebe stejně široké; na 900 px a méně se skládají live -> upload bez horizontálního scrollu. Běžný desktop používá jako svislý scroll owner `.content-area-document`; krátký desktop do výšky 640 px přepne celý shell na jediný dokumentový scroll, aby zůstaly dostupné sidebar i spodní importní akce. Mobilní breakpoint používá také dokumentový scroll. Live režim má caption plochu pro průběžné titulky z realtime přepisu, umí seskupovat live tokeny podle mluvčích, pokud je Soniox vrací, a má přepínač ukládání `Audio do {live limit} + přepis` / `Jen live přepis`. Live titulky se zobrazují s krátkým zpožděním, aby se partial realtime tokeny od Sonioxu stabilizovaly; uložený finální transcript dál vychází ze všech přijatých tokenů, ne jen ze zpožděného caption pohledu. Upload přijímá běžné Soniox async formáty `m4a`, `mp3`, `wav`, `webm`, `ogg`, `flac` a `mp4`. Jeho dropzone a jediný filtrovaný picker vedou do stejného sériového TUS toku a jeden stabilní stavový panel zachovává název, velikost, efektivní limit, byte-weighted progress, finalizaci, úspěch, chybu, zrušení a retry bez vertikálního přeskakování karty. MP4 se může ukládat jako `video/mp4`, ale v aplikaci se dál chová jako nahrávka určená pro Soniox async přepis. Server-only `getRecordingStorageConfig()` čte přes admin klienta explicitní `file_size_limit` i `allowed_mime_types` bucketu `recordings`, kombinuje velikost pouze s omezujícím per-user plan cap a předává výslednou konfiguraci klientským komponentám; globální limit projektu není bezpečně detekovaný. Když limit nebo MIME allowlist nelze načíst, audio cesty jsou vypnuté a textové cesty zůstávají dostupné. Live nahrávání pod vlastním limitem ukládá jeden finální objekt do private Storage; když se audio ukončí dříve kvůli rezervě nebo neprojde finální kontrolou, pokračuje jen přepis. Režim `Jen live přepis` neukládá audio soubor, takže se ho Storage limit netýká; partial transcript se v obou režimech ukládá průběžně do `transcripts`, aby šel záznam obnovit z `/recordings`. Server dál umí číst a znovu přepsat starší segmentované live nahrávky.
...
Stránka `/recordings` je kompaktní inbox nahrávek. Název je hlavní Next odkaz na detail a viditelné tlačítko `Otevřít` se nepoužívá; editor názvu a koš jsou samostatné sourozenecké akce mimo odkaz. Editor neroztahuje řádek a zavře se až po potvrzeném uložení. Změna se zapisuje přes authenticated Supabase klienta a RLS. Panel `Spravovat` pro klienty, projekty, ploché složky a štítky je výchozně zavřený, ale jeho obsah zůstává mounted a ve skrytém stavu je odstraněný z focus/accessibility stromu, aby zavření nezahodilo draft ani pending save; pending settlement proto doběhne i při zavřeném panelu a jeho success/error stav je po znovuotevření zachovaný. Nad seznamem se zobrazí recovery panel, pokud existují nedokončené live drafty s uloženým partial transcriptem nebo audio segmenty. Kompaktní stavová linka počítá všech sedm stavů ze skutečných filtrovaných řádků nebo aktuální stránky search RPC. Seznam má URL parametr `q` pro indexed PostgreSQL hledání v nejnovějším přepisu, názvu nahrávky, klientovi, projektu, složce a štítcích a kanonické filtry `client`, `project`, `folder` a opakovatelný `tag`. Změna organizačních filtrů zachovává `q` i nesouvisející query parametry; vyčištění hledání zachová rozepsané organizační filtry a vyčištění organizace zachová rozepsané `q`. Neplatné, cizí, duplicitní nebo klientovi neodpovídající hodnoty se kanonizují. Více štítků má ALL sémantiku. Vyhledávání vrací stránku 25 výsledků s excerptem a bezpečným odkazem na transcript; bez `q` aplikace načítá organizační seznam stabilním keysetem `(created_at, id)`. Desktop používá kompaktní řádky, viewporty do 900 px skládané karty bez horizontálního posunu. Nahrávku lze přes samostatné tlačítko koše soft-smazat do stavu `deleted`; tím zmizí z hlavního seznamu a objeví se na `/trash`. Koš podporuje trvalé smazání přes server action, která ověří vlastníka, odstraní soubor ze Storage bucketu `recordings` a poté smaže DB řádek; související přepisy, joby, AI výstupy a search chunks se mažou kaskádou. Mazání v UI vždy vyžaduje potvrzení a po potvrzení se řádek nebo explicitně označená search karta optimisticky schová ještě před doběhnutím server action; neočekávaný reject obnoví stejný element a zobrazí sanitizovanou chybu v rezervovaném plnošířkovém druhém řádku uvnitř řádku nebo karty, takže nemění normální 116px action track, tabulka ji neodřízne ani nepřekryje další obsah a nejmenším nutným posunem ji udrží ve viewportu. Route-level loading udržuje geometrii inboxu; route error nezobrazuje interní detail chyby a volá skutečné App Router `reset()`.
...
Koš uchovává původní stav nahrávky v `deleted_from_status` a neměnný čas přesunu v `deleted_at`. Databázový trigger tato metadata atomicky zachytí při přechodu do `deleted`, při obnově vrátí přesný stav a metadata vyčistí. Trvalé smazání je jediná cesta, která odstraní řádek i private Storage objekty; server před použitím service-role klienta ověřuje kanonickou cestu konkrétního uživatele a konkrétní nahrávky. Segmentovaná live cesta smí mít pouze tvar `{user_id}/{recording_id}/live/`. Purge je povolený až po 24 hodinách, protože oficiální Supabase TUS upload URL může zůstat platná až 24 hodin. Authenticated Storage INSERT/UPDATE policy po přesunu do Koše blokuje nové zápisy a purge po odstranění objektů opakovaně prohledá celý adresář nahrávky; DB řádek smaže až po potvrzení, že je prázdný.
