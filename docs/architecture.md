...
V inboxu zůstává hledání vždy viditelné a organizační filtry jsou v keep-mounted pokročilém disclosure. Správa organizace používá keep-mounted pravý Drawer, takže zavření nezahodí draft ani pending save. Stavové chips jsou URL odkazy na serverové facety; samostatné `Smazáno` vede na úplný Koš. Capture workspace zobrazuje storage limity jako jeden informační řádek.

Koš podporuje bounded bulk restore nejvýše 100 položek. Permanentní bulk mazání je klientská fronta: každá položka volá vlastní jednopoložkovou server mutation a nejvýše dvě jsou současně in flight. Zastavení fronty neukončuje již běžící requesty a partial settlement schová pouze úspěšné řádky.
