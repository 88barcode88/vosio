...
- AI detail načítá nejprve bounded metadata, jen výchozí otevřené tělo a další těla až po otevření; AI-inclusive export stránkuje celou historii po 50 metadata řádcích a těla hydratuje v omezených dávkách. Polling je transcript-scoped a běží pouze na aktivní viditelné online ploše `AI zpracování` nebo `Časová osa`: 5 s prvních 30 s, 10 s mezi 30 a 120 s, potom 30 s. Hidden, offline nebo jiná záložka znamená nula requestů; focus/visibility/online dělá jeden deduplikovaný catch-up, po transientní chybě je další pokus nejdříve za 30 s a vždy je nejvýše jeden request in flight. Po dokončení se používá klientský state fetch, ne `router.refresh`.
- Manuální AI joby jsou durable: nový řádek má `execution_mode = manual`, `attempt_count = 0`, `max_attempts = 1` a přesný prompt/model/provider snapshot. Stejný request UUID je idempotentní transportní identita a nerescheduluje provider; explicitní retry používá nové UUID a starý řádek zůstává auditem. Provider volání smí projít pouze přes service-role-only claim s přesným lease tokenem; stale `running` s outputem se uzavře jako `done`, bez outputu jako `failed / execution_interrupted`, nikdy automatickým opakováním.
- AI chyby se normalizují na allowlist `insufficient_credit_or_quota`, `rate_limited`, `invalid_model`, `provider_unavailable`, `provider_configuration`, `execution_interrupted`, `persistence_failed`, `unknown` a volitelné `retry_after_at`. Raw provider message, prompt, transcript, output obsah ani secret se nesmí persistovat, zobrazit ani logovat.
