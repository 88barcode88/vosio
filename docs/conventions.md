...
- AI detail načítá bounded metadata a těla až podle potřeby; export prochází všechny stránky po 50. Polling používá serverové `poll_eligible`, pouze aktivní viditelnou online AI/Timeline plochu a cadence 10 s prvních 30 s, 30 s mezi 30 a 120 s a 60 s poté. Focus/visibility/online udělá deduplikovaný catch-up jen více než 30 s po dokončené kontrole; transientní chyba má backoff nejméně 30 s. Nejvýše jeden request in flight. Aktualizace a lokální mazání nesmějí volat `router.refresh` ani remountovat načtené výstupy. Same-transcript props zachovávají cache; potvrzené mutace používají přesná ID a tombstones, změna transcriptu generation fence.
