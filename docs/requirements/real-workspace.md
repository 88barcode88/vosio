...
AI processing lets the user choose a lower-cost AI model and temperature per run. The model selector includes the model purpose, provider and indicative token pricing for OpenAI and optional Gemini models. The selected model is stored in `ai_processing_jobs.model`, the selected provider in `ai_processing_jobs.provider`, and temperature/output format in `ai_processing_jobs.provider_config`. When an AI run starts, the tab must show a visible running state. The same output type can be started again while an earlier run is still pending, because users may want another pass with different settings.
