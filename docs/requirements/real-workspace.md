...
The complete source schema is the ordered chain `20260617000000_initial_schema.sql` -> evidence `20260804100000` -> organization `20260804110000` -> markers `20260804120000` -> transcript search `20260804130000`. The baseline alone is not the complete source of truth. All four forward migrations are source-only and unapplied to disposable, staging or live Supabase in this work. Release order is evidence, organization, markers, transcript search, successful database postflight on every target database, then application deploy.

No real SQL parse, migration apply, postflight or two-user integration test was run. PostgreSQL 15 syntax, actual evidence/organization/marker/search constraints, trigger and indexes, GIN `EXPLAIN`, grants, forced RLS, anon-vs-auth and cross-tenant isolation, current-vs-old transcript selection, manual/raw/deleted search behavior, runtime keyset/offset pagination and the backfill therefore remain unverified at the database level. Do not deploy application code using any of the four forward contracts before an explicitly approved apply and successful postflight on every target database. Source tests, `npm run check` and production build are not evidence of a successful Supabase migration.

The repository exposes `npm run search:backfill` with explicit disposable/live safety flags, bounded transcript-id keyset batches and a dry-run option. **Do not run it in this plan.** No dry-run or mutating backfill was executed.
