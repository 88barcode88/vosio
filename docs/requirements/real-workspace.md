...
1. The user selects exactly one save mode: `Audio do {efektivní live limit} + live přepis`, `Jen audio do {efektivní live limit}`, or `Jen live přepis`, plus one audio source: `Videohovor: mikrofon + karta` or `Jen mikrofon`.
2. Supported desktop Chrome and Edge with `getDisplayMedia` and Web Audio default to the call source and require the user to select the Google Meet browser tab with `Sdílet také zvuk karty` enabled. The browser retains the required display video track only to keep tab sharing alive, but never records video. Web Audio must reach `running` before capture and mixes the microphone and selected tab audio into one master stream. A canceled picker, non-tab surface, unsupported API, selected surface without audio, ended input, or suspended mixer returns to idle without creating a draft and never silently downgrades to microphone-only. Other desktop browsers and mobile environments keep call capture disabled and default to microphone-only.
3. A session master stream yields isolated archive and Soniox clones; the rotating safety recorder creates its own clone. Provider stop, error, cancel, or restart never owns the archive. If the microphone or shared-tab input ends during active capture, the entire generation stops and finalizes the audio recorded so far instead of continuing with a degraded source.
4. Combined and transcript-only request a short-lived Soniox key and display live tokens. Audio-only skips the realtime key/provider and requests async transcription only after confirmed audio upload.
5. Audio modes use the selected 32, 64, or 96 kbit/s archive/safety quality (14.4, 28.8, or 43.2 decimal MB/hour); Soniox encoding is independent. Mode, source, language, and quality remain locked for the active generation.
6. The archive MediaRecorder, raw microphone and shared-tab mute/unmute/ended state, authoritative mixed-track state, advisory RMS monitor, and provider health are reported separately. One healthy mixed source cannot hide a muted raw input. Unavailable or suspended monitoring Web Audio is monitor-unavailable, while a non-running capture mixer is a startup failure.
7. Every 15 seconds the safety recorder finalizes one part, atomically persists it to IndexedDB, then queues bounded idempotent upload without blocking capture. Normal stop awaits the last part/promotion, finalizes the continuous archive, confirms exact metadata, then removes remote safety parts before local cleanup.
8. Reaching the conservative hard-limit estimate stops and finalizes the whole audio generation with reason `audio_limit`; it never discards audio while continuing realtime. Transcript-only is unaffected.
9. If combined realtime becomes terminally unhealthy, confirmed audio remains primary and async fallback starts only after upload, using restart-safe job creation. Reconnecting alone is nonterminal during capture but requires fallback if still unhealthy at stop.
10. The final transcript is persisted after audio finalization. STT failure never removes confirmed audio; if every audio path fails, the latest visible transcript must settle before the recording is marked failed or the UI must truthfully retain unresolved local state.
...
AI processing selects a stable catalog profile per run: GPT-5.6 Sol/Terra/Luna, Gemini 3.6/3.8 Flash, Astra Low/Medium and Mistral Small 4/Large 3. `ai_processing_jobs.model` stores the profile, `provider` the provider, and `provider_config.provider_model` snapshots the exact API model with effective reasoning/thinking. Both Astra profiles call `gpt-6-astra`. Legacy snapshots without this field use validated catalog mapping; explicit mismatches fail closed. Unknown saved model preferences disable automatic consent instead of starting a paid default-provider call. Model labels and indicative pricing come from `src/lib/model-options.ts`; provider billing is authoritative. Mistral supports temperature, the other current profiles do not. Show pending state for each new UUID; independent runs of the same output type are allowed. Users must verify important extracted facts against the transcript.
...
Recording detail does not query AI bodies/projections on the server critical path. Transcript-scoped lazy state pages metadata and hydrates bodies on demand; exports walk the complete history. Same-transcript props preserve loaded state and mounted content. Exact confirmed deletion IDs create local tombstones; absence from a bounded page is not deletion. Polling uses server `poll_eligible`: 10 seconds for the first 30 seconds, 30 until 120 seconds, then 60. It pauses on hidden/offline/irrelevant tabs, deduplicates lifecycle bursts and checks on return only more than 30 seconds after the last completed check. At most one request, error backoff at least 30 seconds, no full router refresh. The job panel starts closed, hides done rows and offers only server-authorized actions. Bulk cleanup walks GET/POST pages of at most 50 exact IDs and preserves jobs with outputs or projections.
