...
Nulový výsledek je nutná, nikoli postačující podmínka apply. Jakýkoli řádek blokuje live apply a vyžaduje explicitní lineage review každého outputu. Migrace stejný invariant kontroluje fail-closed ještě před unique indexem a vyhodí výjimku; neobsahuje blind dedup ani delete. Private Vosio tento preflight úspěšně dokončil před ručním apply 2026-08-27; každý jiný target zůstává `live apply blocked pending preflight`, dokud není doložen jeho vlastní nulový výsledek a schválen další rollout krok.
