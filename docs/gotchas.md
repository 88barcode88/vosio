...
Polling `ai-state` je omezený na aktivní relevantní tab `AI zpracování` nebo `Časová osa`, a to pouze když je dokument visible a online. Cadence je 5 s prvních 30 s, 10 s mezi 30 a 120 s a 30 s poté; hidden/offline/jiný tab znamená nula. Lifecycle burst se sloučí do jednoho catch-up, po transientní chybě je backoff nejméně 30 s a nikdy neběží více než jeden request. Po terminalním reconcile se stav obnoví klientským fetchem bez `router.refresh`.

Revalidace detailu (například po uložení názvu nebo smazání AI výstupu) předává znovu prázdné lazy AI props. Provider proto při invalidaci mění `stateRevision` a aktivní AI/Timeline plocha znovu načte aktuální metadata i potřebné tělo. Samotný metadata poll tuto revizi nemění. Neodstraňovat invalidaci pouze podle props: v cache by zůstaly smazané výstupy a jejich projekce. Hidden/offline plocha odloží první načtení do návratu; staré odpovědi nesmějí smazat deduplikační stav nového requestu.
